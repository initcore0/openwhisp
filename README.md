...
- Engine (Parakeet · WhisperKit · Whisper · Apple Speech), raw model picker + paths, live‑chunk tuning, whisper.cpp backend (CLI vs warm server), per‑app modes, history, permissions, diagnostics. The settings shown adapt to the selected engine.
...
- **Parakeet backend (default)** — a Swift‑native CoreML streaming runtime built on NVIDIA Parakeet models via [FluidAudio](https://github.com/FluidInference/FluidAudio), now the **default transcription engine** on a fresh install. True streaming: words appear ~0.32 s behind your voice, with punctuation, and `finish()` returns in ~50 ms (no batch decode) — the crispest realtime dictation of the bundled engines. English streaming variants plus a ~40‑language multilingual variant; background/file jobs (meetings, queue, watch folders) run on Parakeet TDT v3. Compiled in by default; a lean `PARAKEET=0 ./build.sh` hides it and the default falls back to WhisperKit. Existing installs keep the engine they already chose (`SettingsMigration` v4). Full technical write‑up: [docs/PARAKEET.md](docs/PARAKEET.md).
- **WhisperKit backend** — a Swift‑native CoreML/ANE Whisper runtime ([Argmax](https://github.com/argmaxinc/WhisperKit)). It does both file transcription and **real‑time streaming** (built‑in VAD skips silence) and shares the streaming session path with Apple Speech. Compiled in by default and the **Recommended** engine on a lean `PARAKEET=0` build; build lean without it via `WHISPERKIT=0 ./build.sh` (the engine then falls back to whisper.cpp). Two macOS‑26 gotchas worth knowing: models are loaded from a locally‑staged folder of compiled `.mlmodelc` (the published prebuilts don't load on Tahoe), and the audio encoder is pinned to the **GPU** because ANE specialization of a non‑tiny encoder can stall indefinitely. Settings become backend‑aware (only the relevant options show per engine). Full technical write‑up: [docs/WHISPERKIT_PILOT.md](docs/WHISPERKIT_PILOT.md).
