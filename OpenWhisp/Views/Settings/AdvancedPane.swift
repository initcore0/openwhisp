...
struct AdvancedPane: View {
    ...
    private var diagnosticsSection: some View {
        Section {
            // Which build is running — version + the exact commit it was built
            // from (BuildInfo.swift is generated by build.sh at compile time).
            // "+" after the commit means uncommitted changes were present.
            LabeledContent("Build") {
                Text("\(Bundle.main.infoDictionary?["CFBundleShortVersionString"] as? String ?? "dev") · \(BuildInfo.display)")
                    .foregroundColor(.secondary)
                    .textSelection(.enabled)
            }
